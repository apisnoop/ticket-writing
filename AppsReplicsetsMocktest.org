# -*- ii: apisnoop; -*-
#+TITLE: Replicasets Apps endpoints
#+AUTHOR: ii team
...
*** 1. Create a replicaset yaml file, namespace and replicaset
...
***  2. Find the replicaset
#+begin_src shell :results raw
  kubectl get replicaset -A | grep app
#+end_src

#+RESULTS:
#+begin_example
app-replicaset-tests   frontend                            10        10        10      21m
#+end_example

WIP - Updated the file to this points for replicaset test, copied form deployment test

*** 3. Scale the results

#+begin_src shell :results raw
  kubectl scale --replicas=4 rs/frontend -n app-replicaset-tests
  sleep 5
 kubectl get replicas -A | grep app
 kubectl rollout status frontend -n app-replicaset-tests
#+end_src
...
*** 4. Update the replicaset: NOT WORKING
#+begin_src shell :results raw
kubectl set image frontend -n app-replicaset-tests app=guestbooks --record
#+end_src
...
*** 5. Describe the replicaset to see if it was updated with history - NOT WORKING
#+begin_src shell :results raw
  kubectl describe replicas -n app-replicaset-tests | grep image
  kubectl rollout history frontend -n app-replicaset-tests
#+end_src
